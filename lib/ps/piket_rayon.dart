...
import 'package:scr_wikrama/ps/pplg_rooms.dart';
import 'package:scr_wikrama/ps/rekap_piket.dart';

class PiketRayon extends StatefulWidget {
  const PiketRayon(
      {super.key, required this.userLogin, required this.id_ruang});

  final List userLogin;
  final dynamic id_ruang;

  @override
  State<PiketRayon> createState() =>
      _PiketRayonState(dataUser: userLogin, id_ruang: id_ruang);
}

class _PiketRayonState extends State<PiketRayon> {
  _PiketRayonState({required this.dataUser, required this.id_ruang});
  ...
  Future<void> _getDataRooms() async {
    var url =
        Uri.http("127.0.0.1", "/scr_wikrama/getDataRooms.php", {'q': '{http}'});
    ...
    Navigator.push(
        context, MaterialPageRoute(builder: (context) => PPLGRooms(
          rooms: rooms,
        )));
  }

  Future<void> _displaySiswa() async {
    // tampilkan data siswa per rayon
    ...
    var url = // server lokal
        Uri.http("127.0.0.1", "/scr_wikrama/students/displayStudents.php",
            {'q': '{http}'});
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) => DisplayStudent(students: students)),
      );
    } else {
      print("Koneksi gagal!");
    }
